# vite-react-spa-template · React 前端底座

Vite + React + TypeScript SPA 底座模板（copier），AI Foundation 系列的前端底座之一。
...
- `_envops` 把 jinja 标签改为 `[[ ]]` / `[% %]` / `[# #]`，避开 JSX 的 `{{ }}`
- 需渲染文件加 `.jinja` 后缀 + `[[ ]]` 占位符（本项目 5 个：constants / index.html / .env / docs/index.md / package.json）；其余文件纯复制
...
本模板是 AI Foundation 系列的前端底座（React），与 `vite-vue-spa-template` 双模板直接对齐。系列元信息（目的 / 设计思路 / 姊妹底座 / copier 位置）见 [CLAUDE.md](./CLAUDE.md) 导入的系列全貌。
