...
import type { AppRequestConfig } from '@/api/types/http'
 ...
 * 拦截器只做 token 生命周期，不导航——跳登录由外层（守卫 / QueryCache onError）负责。
 */
export function attachAuth(httpInstance: AxiosInstance): void {
  ...
  httpInstance.interceptors.response.use(
    ...
    async (error: AxiosError<AuthErrorPayload>) => {
      ...
      // 401 + 40103：会话失效 → 清 token，reject 透传错误码（外层 QueryCache onError 拦截 40103 跳登录）
