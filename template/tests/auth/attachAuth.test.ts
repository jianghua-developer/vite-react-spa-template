...
function makeAuthError(status: number, code: string, refreshToken?: string) {
  ...
  err.config = { url: '/protected', headers: {}, authRequired: true }
  return err
}
...
describe('attachAuth', () => {
  ...
  it('authRequired 请求注入 x-access-token', () => {
    setToken('token-1')
    const config = hooks.req({ headers: {}, authRequired: true })
    expect((config.headers as Record<string, string>)['x-access-token']).toBe('token-1')
  })

  it('非 authRequired 请求不注入 token', () => {
    const config = hooks.req({ headers: {} })
  ...
  it('401+40102 → 用后端下发的 refresh_token 无感刷新并重放', async () => {
    ...
    // 用 401 响应里的 refresh_token 刷新（覆盖存储）
